...
static void initCompiler(Compiler* compiler, FunctionType type) {
	...
	compiler->loop.loopStart = -1;
	initIntArray(&compiler->loop.breaks);
	initIntArray(&compiler->loop.continues);
	current = compiler;
...
static void breakStatement() {
	if (current->loop.loopStart == -1) {  // 使用loopStart来检测是否在循环中
		error("Break statement can only be used inside a loop.");
		return;
	}
	emitByte(OP_POP); // Pop the condition if any.
	int breakJump = emitJump(OP_JUMP);
	writeIntArray(&current->loop.breaks, breakJump);
...
static void continueStatement() {
	...
	writeIntArray(&current->loop.continues, continueJump);
...
static void switchStatement() {
	...
	expression(); // This evaluates the switch expression
	consume(TOKEN_RIGHT_PAREN, "Expect ')' after switch expression.");
	consume(TOKEN_LEFT_BRACE, "Expect '{' before switch cases.");

	int oldJump = -1;
	int jumpList[256]; // Arbitrary size. Be careful with overflows.
	int jumpCount = 0;
	while (!match(TOKEN_RIGHT_BRACE) && !check(TOKEN_EOF)) {
		if (match(TOKEN_CASE)) {
			...
			expression(); // This evaluates the case expression
			consume(TOKEN_SEMICOLON, "Expect ':' after case value.");
			int caseJump = emitJump(OP_JUMP_IF_NOT_EQUAL);
			jumpList[jumpCount++] = caseJump;
			emitByte(OP_POP); // Pop the switch expression
			statement();
			oldJump = emitJump(OP_JUMP); // Jump to the end of switch
		}
		else if (match(TOKEN_DEFAULT)) {
			consume(TOKEN_SEMICOLON, "Expect ':' after default.");
			if (oldJump != -1) {
				patchJump(oldJump);
			}
			statement();
	...
	for (int i = 0; i < jumpCount; i++) {
		patchJump(jumpList[i]);
		emitByte(OP_POP); // Pop the case expression
	}
...
static void forStatement() {
	...
	// 初始化loop markers
	current->loop.loopStart = currentChunk()->count;
	current->loop.loopExit = -1;
	initIntArray(&current->loop.breaks);
	initIntArray(&current->loop.continues);
	...
	// 修复所有的break和continue跳转
	for (int i = 0; i < current->loop.breaks.count; i++) {
		patchJump(current->loop.breaks.values[i]);
	}
	for (int i = 0; i < current->loop.continues.count; i++) {
		patchJump(current->loop.continues.values[i]);
		emitByte(OP_POP);
		emitLoop(current->loop.loopStart);
	}

	freeIntArray(&current->loop.breaks);
	freeIntArray(&current->loop.continues);
